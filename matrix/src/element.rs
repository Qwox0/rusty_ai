...
pub trait Element: Copy + Default + fmt::Debug + fmt::Display + Send + Sync
    + SampleUniform // Remove this?
    + 'static
{
}

impl<T: Copy + Default + fmt::Debug + fmt::Display + Send + Sync + 'static + SampleUniform> Element
    for T
{
}

pub trait Num:
    Element + num::ToPrimitive + num::FromPrimitive + num::Num + num::NumCast + MoreNumOps
{
    const ZERO: Self;
    const ONE: Self;
    const MAX: Self;
