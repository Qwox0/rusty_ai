...
use const_tensor::{matrix, Matrix, MultidimensionalOwned, Vector};
...
macro_rules! bench_mul_vec {
    ( $( $bench_name:ident : $dim:expr ),* $(,)? ) => { $(
        ...
        fn $bench_name(b: &mut Bencher) {
            ...
            let matrix = Matrix::<f64, $dim, $dim>::from_iter(&mut rng_iter);
            let vec = Vector::<f64, $dim>::from_iter(&mut rng_iter);
...
macro_rules! bench_alloc_identity {
    ( $( $bench_name:ident : $ty:ty => $dim:expr ),* $(,)? ) => { $(
        #[bench]
        fn $bench_name(b: &mut Bencher) {
            b.iter(|| black_box(Matrix::<$ty, $dim, $dim>::identity()))
        }
    )* };
}
...
macro_rules! bench_from_arr {
    ( $( $bench_name:ident : $ty:ty => $dim:expr ),* $(,)? ) => { $(
        #[bench]
        fn $bench_name(b: &mut Bencher) {
            b.iter(|| black_box(Matrix::<$ty, $dim, $dim>::new(black_box([[<$ty>::default(); $dim]; $dim]))))
        }
    )* };
}
